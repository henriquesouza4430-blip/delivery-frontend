...
const PLAN_DATA: Record<string, PlanMeta> = {
  BASIC: {
    ...
    features: [
      ...
      "Frente de Caixa (PDV) & Controle de caixa",
      "Pedidos online, Cupons e Taxa por bairro",
    ],
  },
  PRO: {
    label: "Profissional",
    ...
    features: [
      "WhatsApp automático & Status de pedidos",
      "Gestão de entregas, Motoboys e KDS",
      "Estoque, Ficha técnica e PIX automático",
    ],
  },
  ENTERPRISE: {
    label: "Premium",
    ...
    description: "Operação completa e automatizada",
    features: [
      "IA completa no WhatsApp",
      "Integração iFood e 99 Food",
      "Emissão de Nota Fiscal & DRE Gerencial",
    ],
  },
};
...
function waLink(plan?: string): string {
  const planLabel = plan ? (PLAN_DATA[plan]?.label ?? plan) : "";
  const text = plan
    ? `Olá! Tenho interesse no plano ${planLabel} da ZH Sistema de Delivery. Gostaria de receber uma proposta comercial.`
    : `Olá! Gostaria de saber mais sobre a ZH Sistema de Delivery e entrar em contato com um consultor.`;
      ...
      >
        <div className="max-w-2xl mx-auto flex items-center justify-between gap-3">
          <span className="text-[10px] text-white/20 hidden md:block">
            © {new Date().getFullYear()} Ruffinu&apos;ZH Sistema de Delivery
          </span>
    ...
    <div className={`rounded-2xl bg-gradient-to-br ${data.gradient} p-[1px]`}>
      <div className="rounded-[15px] bg-[#0a0a1a] px-4 py-3 space-y-2.5">
        ...
        <div className="flex gap-2 pt-0.5">
          ...
          <a
            href={`https://wa.me/${SUPPORT_WHATSAPP}?text=${encodeURIComponent(`Olá! Tenho interesse no plano ${data.label} da ZH Sistema de Delivery. Gostaria de receber uma proposta comercial.`)}`}
            target="_blank"
            rel="noopener noreferrer"
            onClick={onWaClick}
