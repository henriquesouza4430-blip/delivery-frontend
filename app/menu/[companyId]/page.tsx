...
export default function MenuPage() {
  ...
  // ── ESTADO PARA ARMAZENAR OS HORÁRIOS DE FUNCIONAMENTO ────────
  ...
  async function loadMenu(attempt = 1) {
    ...
    try {
      ...
      if (companyRes?.ok) {
        ...
        if (cd?.businessHours) setBusinessHours(cd.businessHours); // <--- POPULANDO OS HORÁRIOS AQUI
      }
  ...
  async function validateCoupon(code: string) {
    ...
    try {
      ...
      // CORRIGIDO: de 'finaly' para 'finally' para matar o erro de sintaxe do TS!
  ...
  return (
    <div className="min-h-screen bg-gray-50">
     ...
     <header className="relative bg-neutral-900 text-white pb-6 pt-4 px-4 rounded-b-3xl shadow-lg overflow-hidden">
        {/* Banner de Fundo */}
        {theme?.bannerUrl && (
          <div className="absolute inset-0 z-0 opacity-40 mix-blend-multiply">
            <img src={theme.bannerUrl} alt="Banner" className="w-full h-full object-cover" />
          </div>
        )}
        ...
        <div className="relative z-10 flex flex-col items-center text-center mt-4">
          {/* Logo */}
          {theme?.logoUrl && (
            <div className="w-20 h-20 rounded-full border-2 border-white/20 bg-neutral-800 p-1 mb-3 overflow-hidden shadow-md">
              <img src={theme.logoUrl} alt={companyName} className="w-full h-full object-cover rounded-full" />
            </div>
          )}
          ...
          <div className="flex items-center gap-4 mt-3 text-white/80 text-xs">
            {(() => {
              if (!businessHours) {
                return (
                  <span className="flex items-center gap-1 text-red-400 font-semibold bg-black/20 px-2 py-0.5 rounded">
                    <Clock size={12} /> Fechado (Sem horário cadastrado)
                  </span>
                );
              }
              ...
              try {
                ...
                if (!hoursConfig || !hoursConfig.isOpen) {
                  return (
                    <span className="flex items-center gap-1 text-red-400 font-semibold bg-black/20 px-2 py-0.5 rounded">
                      <Clock size={12} /> Fechado agora
                    </span>
                  );
                }

                const currentMinutes = now.getHours() * 60 + now.getMinutes();
                const [openHours, openMinutes] = hoursConfig.open.split(':').map(Number);
                const [closeHours, closeMinutes] = hoursConfig.close.split(':').map(Number);
                ...
                if (closeTimeInMinutes < openTimeInMinutes) {
                  isOpenNow = currentMinutes >= openTimeInMinutes || currentMinutes <= closeTimeInMinutes;
                } else {
                  isOpenNow = currentMinutes >= openTimeInMinutes && currentMinutes <= closeTimeInMinutes;
                }
                ...
                if (isOpenNow) {
                  return (
                    <span className="flex items-center gap-1 text-green-400 font-semibold bg-black/20 px-2 py-0.5 rounded">
                      <Clock size={12} /> Aberto agora
                    </span>
                  );
                }
